...
| A plane judges nothing until it is filled: on two layers a tap the pour will not reach is refused before it is written, and on every board the gate's own filled copper is checked back — each plane still one island, each tap via inside its own | `test_patterns.py::test_a_tap_the_pour_cannot_reach_is_refused_before_it_is_written`, `test_the_plane_a_tap_lands_in_is_still_one_island`, `test_examples_fab.py::test_example_builds_to_fab` |
