...
def place_job(design: Design, seed: Path, *, out: Path) -> dict:
    ...
    for round_ in range(3):
        ...
        silk = silk_job(job, out, out=out, backup=False)["silk"]  # references placed here; fab and review re-run it, same answer
        cramped = {m.split(":")[0] for m in silk.get("issues", [])}
        retry = [s for s in specs if s.ref in cramped and s.to]
